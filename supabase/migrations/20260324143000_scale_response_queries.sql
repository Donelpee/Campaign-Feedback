-- Improve response browsing scalability with targeted indexes and server-side helpers

CREATE INDEX IF NOT EXISTS idx_feedback_responses_link_id
  ON public.feedback_responses (link_id);

CREATE INDEX IF NOT EXISTS idx_feedback_responses_created_at_desc
  ON public.feedback_responses (created_at DESC);

CREATE INDEX IF NOT EXISTS idx_feedback_responses_campaign_created_at_desc
  ON public.feedback_responses (campaign_id, created_at DESC);

CREATE OR REPLACE FUNCTION public.jsonb_has_answer_value(value JSONB)
RETURNS BOOLEAN
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    ...
    WHEN jsonb_typeof(value) = 'string' THEN length(btrim(value #>> '{}')) > 0
    ELSE true
  END
$$;
...
AS $$
  WITH scoped_rows AS (
    SELECT
      ...
      camp.questions AS campaign_questions,
...
CREATE OR REPLACE FUNCTION public.get_campaign_response_counts(
  p_company_id UUID DEFAULT NULL
)
RETURNS TABLE (
  campaign_id UUID,
...
AS $$
  SELECT
    c.id AS campaign_id,
    COUNT(fr.id)::BIGINT AS response_count
  FROM public.campaigns c
  LEFT JOIN public.company_campaign_links ccl ON ccl.campaign_id = c.id
  LEFT JOIN public.feedback_responses fr ON fr.link_id = ccl.id
  WHERE public.has_campaign_access(auth.uid(), c.id)
    AND (
      p_company_id IS NULL
      OR (
        ccl.company_id = p_company_id
